...
Class AepRu extends simpleParser {
    ...
    public $cookieFile = '';
    public $donor = 'AepRu';
    public $project = 'archive.aep.ru';
    public $project_link = 'archive.aep.ru';
    ...
    {
        $sources = [];
        $content = $this->loadUrl('http://archive.aep.ru/wps/wcm/connect/aep/main/presscenter/nnews/', $var);
        if ( !$content )
        {
            return $sources;
        }
        if (preg_match('%url:[\s]*\'([^\']+)\'%uis', $content, $match))
        {
            ...
            $content = $this->loadUrl('http://archive.aep.ru/wps/wcm/connect/aep/main/presscenter/nnews/'.$hash, $var);
            if ( !$content )
            {
                return $sources;
            }
            ...
            foreach ($sourceArray as $item)
            {
                ...
                $hash = $uri[count($uri)-1];
                $sources[] = [
                    'source' => 'http://archive.aep.ru/wps/wcm/connect/aep/main/presscenter/nnews/',
                    'href' => "http://archive.aep.ru/wps/wcm/connect/aep/main/presscenter/nnews/".$hash,
                    'title' => $item->title,
    ...
    public function getNewsDomMethod($url, $source = [])
    {
        ...
        $content = preg_replace('%<h3[^>]+class="raTitleNews[^<]*</h3>%uis','',$content);
        $content = preg_replace('%<div[^>]*id="newsDate.*?</div>%uis','',$content);
        $content = preg_replace('%<div[^>]*id="materialAttachBlock[^<]*</div>%uis','',$content);
